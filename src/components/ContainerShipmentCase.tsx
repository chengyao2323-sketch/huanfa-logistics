...
export default function ContainerShipmentCase({ onOpen }: { onOpen: (image: ProofImage) => void }) {
  ...
  const copy = zh ? {
    ...
    note: "15天为本票从中国起运至美国目的地交付的实际全程时效，并非仅海上航程。不代表其他货件的时效承诺；具体船期及预计全程时效以订舱确认为准。",
    ...
    facts: [
      ...
      ["End-to-end delivery time", "15 days"],
      ["Shipment month", "July 2026"],
    ],
    note: "15 days was the actual end-to-end delivery time from dispatch in China to delivery at the US destination, including the ocean journey and final delivery. This completed shipment is not a guarantee for other cargo; sailing schedules and estimated end-to-end timing are confirmed at booking.",
