              ...
              {[
                ...
                <div key={item.label} className="flex items-start gap-3 rounded-xl border border-gray-100 bg-gray-50 p-4">
                  ...
                  <div>
                    ...
                    <div className="break-words text-sm text-gray-600">{item.label === t.homeSections.verification.email ? <a href={`mailto:${item.value}`} className="underline">{item.value}</a> : item.value}</div>
                  </div>
                </div>
              ))}
            </div>
